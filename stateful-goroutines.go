...
func main() {
  ...
  // the read and write channels will be usedby other
  // goroutines to issue read and write requests

  reads := make(chan *readOp)
  writes := make(chan *writeOp)

  // this is the goroutine that ownes the state
  // it selects on the reads and writes Channels
  ...
  // goroutine owning the state via the reads channel
  ...
  for r := 0; r < 100; r++ {
    ...
    // now 10 writes using the same approach
    ...
    for w := 0; w < 10; w++ {
      ...
      // let goroutines work for 1 second
